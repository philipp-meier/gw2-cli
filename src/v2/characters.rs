...
pub async fn print_character_stats(client: &Gw2Client, name: &str) -> Result<(), Gw2ApiError> {
    match CharacterCore::get(client, name).await {
        Ok(character) => {
            let rows = vec![
                StatsRow::new("Title", &character.name),
                StatsRow::new("Separator", "-----------------------"),
                StatsRow::new("Age", &get_age_from_create_date(character.created)),
                StatsRow::new("Race", &character.race),
                StatsRow::new("Gender", &character.gender),
                StatsRow::new("Profession", &character.profession),
                StatsRow::new("Level", &character.level.to_string()),
                StatsRow::new("Deaths", &character.deaths.to_string()),
                StatsRow::new("Playtime", &get_age_from_seconds(character.age.into())),
            ];
            print_stats(rows);
        }
